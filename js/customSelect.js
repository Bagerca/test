export class CustomSelect {
    ...
    addOption(author) {
        ...
        if (author.handle === this.selectedValue) {
            li.classList.add('selected');
        }

        const avatarSrc = author.avatarUrl || this.fallbackAvatar;
        
        // Разметка отдельного пункта меню
        li.innerHTML = `
            ${author.handle === 'all' 
                ? `<div class="custom-select-avatar-placeholder svg-icon">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                   </div>` 
                : `<img src="${avatarSrc}" alt="Avatar" class="custom-select-avatar" onerror="this.src='${this.fallbackAvatar}'">`
            }
            <div class="custom-select-text">
                <span class="author-handle">${author.handle === 'all' ? 'Все разработчики' : author.handle}</span>
            </div>
        `;

        li.addEventListener('click', () => {
            this.selectValue(author, li);
        });
    ...
    selectValue(author, liElement) {
        ...
        // Обновляем текст и аватар на кнопке
        this.currentValueEl.textContent = author.handle === 'all' ? 'Все разработчики' : author.handle;
        
        if (author.handle === 'all') {
            this.currentAvatarEl.innerHTML = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>`;
            this.currentAvatarEl.classList.add('svg-icon');
        } else {
            const avatarSrc = author.avatarUrl || this.fallbackAvatar;
            this.currentAvatarEl.innerHTML = `<img src="${avatarSrc}" alt="Avatar" class="custom-select-avatar" onerror="this.src='${this.fallbackAvatar}'">`;
            this.currentAvatarEl.classList.remove('svg-icon');
        }

        // Обновляем классы выделения
        const allOptions = this.dropdown.querySelectorAll('li');
        allOptions.forEach(opt => opt.classList.remove('selected'));
        liElement.classList.add('selected');

        this.close();
        
        // Вызываем коллбек для фильтрации ленты
        if (this.onChange) {
